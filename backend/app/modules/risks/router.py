...
from app.modules.risks.schemas import RiskBulkDeleteRequest, RiskBulkDeleteResponse, RiskBulkPublishRequest, RiskBulkPublishResponse, RiskItem, RiskParseRequest, RiskParseResponse, RiskUpdate
...
@router.post("/bulk-delete", response_model=RiskBulkDeleteResponse)
def bulk_delete_risks(payload: RiskBulkDeleteRequest, current_user: SeedUser = Depends(get_current_user)) -> RiskBulkDeleteResponse:
    ...
    for risk_id in dict.fromkeys(payload.risk_ids):
        ...
        if delete_risk(risk_id):
            deleted_ids.append(risk_id)
    return RiskBulkDeleteResponse(deleted_ids=deleted_ids, skipped=skipped)


@router.post("/{risk_id}/publish", response_model=RiskItem)
def publish(risk_id: str, current_user: SeedUser = Depends(get_current_user)) -> RiskItem:
    risk = get_risk(risk_id)
    if risk is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Risk not found")
    if get_project_for_user(risk.project_id, current_user) is None:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Project access denied")
    published = publish_risk(risk_id)
